...
config_file_path="/home/parmardee/deepak/Mtech_project/new_task_1/dynamic_scene_model2/model2/model2.yml"
...
sys.stdout = open('/home/parmardee/deepak/Mtech_project/new_task_1/DICHASUS_dataset/model_DICHASUS_logfile.txt', 'w')
...
#     save_path = os.path.join(processed_folder, f'sample_1_input.npy')
#     if os.path.exists(save_path):
#         print(f"Skipping {save_path}, already processed.",flush=True)
#         continue
...
print(f"Train data: {len(train_dataset)}, Val data: {len(val_dataset)},Test data: {len(test_dataset)}",flush=True)
...
class CNN_LSTM_Net(nn.Module):
    def __init__(self, **kwargs):
        ...
        # # self.test_losses = []  
        # self.avg_train_distances = []
        # self.avg_val_distances = []
        # self.train_dist_sum = 0
        ...
        # LSTM
        # self.lstm1 = nn.LSTM(input_size=36*self.no_anteena, hidden_size=400, batch_first=True)
        # self.ln = nn.LayerNorm(400)
        # self.lstm2 = nn.LSTM(input_size=400, hidden_size=100, batch_first=True)
    ...
    def forward(self, x1, x2):   
        ...
        # print(x1_fisrt.shape) #------[64,36,8,8]
        # print(x1.shape) #----[64, 5,64,33]
          # (batch, 36*8*4)      
        # LSTM
        x1_real = x1.real     
        x1_imag = x1.imag 
        ...
        # print(x1_fisrt.shape)
        # print(x1_second.shape)
        # print(aa)
...
#     l2 = torch.norm(pred - target, dim=1).mean()
#     cosine = cos(pred, target).mean()
#     cosine_loss = 1 - cosine
#     return 0.5 * l2 + 0.5 * cosine_loss



def custom_loss(mu, Sigma, target, 
                         alpha = 1, beta = 0.2, 
                         weight_1=0.4, weight_2=0.6):
    """
...
#     # Euclidean loss
#     l2_loss = torch.norm(pred - target, dim=1).mean()

#     return alpha * uncertainty_loss + beta * l2_loss


# def custom_loss(mu, Sigma, target, alpha=0.7, beta=0.3):
#     diff = (target - mu).unsqueeze(-1)  # [B, 3, 1]
#     inv_Sigma = torch.linalg.inv(Sigma)
#     mahalanobis = torch.matmul(diff.transpose(1, 2), torch.matmul(inv_Sigma, diff)).squeeze()
#     log_det = torch.logdet(Sigma + 1e-6 * torch.eye(3, device=Sigma.device))
#     return (alpha*mahalanobis + beta*log_det)
...
def euclidean_distance(pred, target):
    ...
    # return torch.sqrt(torch.sum((pred - target) ** 2, dim=1))*100
...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=lr_factor, patience=lr_patience, eps=lr_eps)
...
for epoch in range(1, epochs + 1):
    ...
    for x1, x2, y in train_loader:
        if x1.shape != expected_input_shape:
            # print(f"Skipping sample due to shape mismatch: got {x1.shape}",flush=True)
            continue
        ...
        cov_params  = output[:, 3:]        
        L = torch.zeros((128, 3, 3), device=device)
        ...
        loss = loss.mean() 
        # loss = custom_loss(pred_coords , y)
        errors_m = torch.norm(pred_coords - y, dim=1)
        train_error+=errors_m.mean().item()
        loss.backward()
        optimizer.step()
        train_loss += loss.item()
    epoch_train_loss.append(train_loss/len(train_loader))
    epoch_train_error.append(train_error/len(train_loader))
    ...
    with torch.no_grad():
        ...
        # errors_m = torch.norm(all_preds - all_labels, dim=1)*100
        errors_m = torch.norm(all_preds - all_labels, dim=1)
        mean_val_error_m = errors_m.mean().item()
        if(epoch%5==0):
            print(f"Mean val error at {epoch}: {mean_val_error_m:.2f} m", flush=True)
            print(f"Avg val standered deviation in [x,y,z] = {torch.sqrt(torch.diag((torch.mean(all_sigma, dim=0))))}")
        mean_val_error_m_list.append(mean_val_error_m)
    scheduler.step(val_loss)
    mean_val_error_m=0
    print(f"Val Loss = {val_loss/len(val_loader):.4f}",flush=True)
    
    if val_loss/len(val_loader) < best_average_loss:
        best_average_loss = val_loss/len(val_loader)
        torch.save(model.state_dict(), model_save_path)
        print(f"New best model saved with average loss {val_loss/len(val_loader):.4f}",flush=True)

save_train_eval_plot(epoch_train_loss, epoch_val_loss, mean_val_error_m_list, epoch_train_error,
                    save_path=f"{result_path}/loss_plot.png")
